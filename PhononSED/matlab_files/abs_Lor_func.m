function varargout = abs_Lor_func(x,y,varargin)
...
switch lower(nparams)
    ...
    case '3'
        % Define P0, LB, UB
        if isempty(p0);
            error 'P0 must three elements for NPARAMS = ''3'''; 
        elseif numel(p0)~=3
...
otherwise
    warning('MATLAB:mod_lorentzfit:default','Fitting default (''3c'') Lorentzian model.')
    [params,resnorm,residual] = lsqcurvefit(@lfun3c,p0,x,y,lb,ub,options);
    yprime = lfun3c(params,x);
end
...
F = (p(1)*(p(2)^2+p(3))*(p(3)^0.5/(4*pi)))./((x-p(2)).^2+p(3)) + p(4);
end % LFUN3C
