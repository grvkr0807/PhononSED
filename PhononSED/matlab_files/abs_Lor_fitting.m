...
type= 'abs_Lor_fit';    %fitting algorithm 
...
for kk= 1:Nk
    ...
    w0= w0*c_light*2*pi;
    w0(1:3)= 1e-6*[1;1;1];   % zero frequency for acoustic modes
    ...
    w_mid= zeros(1,Neig);  % to store midpoint frequency of Lorentzian
...
for i=1:Neig
        ...
        SED_part= (SED(index(1,i):index(2,i),i));
        
        SED_rms(kk,i)= rms(log(SED_part));
        P2= peak_freqs(i);               
        P3= 0.5*(freqs(ind_mid(2,i)) - freqs(ind_mid(1,i)));
        P1= (max_SED(i))*4*pi*P3^0.5/(P2^2+P3);          
        P0= [P1,P2,P3];    %parameters for Lorentzian fit
        BOUNDS= [0,0,0;Inf,Inf,Inf];
        [SED_fit(index(1,i):index(2,i),i), params(:,i), resnormal(i), residual(index(1,i):index(2,i),i)] = SEDfit_func(freqs(index(1,i):index(2,i)),SED_part,P0,BOUNDS,'abslor');
        norm_error(i)= rms(log(SED_part) - log(SED_fit(index(1,i):index(2,i),i)))/ SED_rms(kk,i);
        [a,b]=max(SED_fit(:,i));
