function varargout = log_Lor_func(x,y,varargin)
...
p3 = 1e-2;
c = 0;
p1 = (max_y-c)*4*pi*p3^0.5/(p2^2+p3); 

optargs = {[],[],'3c',optimset('TolFun',max(mean(y(:))*1e-16,1e-25),'TolX',max(mean(x(:))*1e-16,1e-25))};
...
switch lower(nparams)
    ...
    case '3'
        % Define P0, LB, UB
        if isempty(p0);
            error 'P0 must have three elements for NPARAMS = ''3'''; 
        elseif numel(p0)~=3
...
% F = log((p(1)*p(3).^0.5*(p(2).^2+p(3))./(4*pi*((x-p(2)).^2 + p(3)))));
% end % LFUN3
...
F = (log(abs(p(1))) + 0.5*log(abs(p(3))) +log(abs(p(2)^2+p(3))) - log(abs((x-p(2)).^2+p(3))) -log(4*pi)) + p(4);
end % LFUN3C
