...
type= 'Log_full_fit';    %fitting algorithm 
...
for kk= 1:Nk
    ...
    k_no= num2str(kk); % kpoint index converted to string for file naming
...
for i=1:Neig
        ...
        P3= peak_freqs(i);               
        P4= 0.5*(freqs(ind_mid(2,i)) - freqs(ind_mid(1,i)));
        P1= (max_SED(i))/((P3^2+P4)/P4^0.5 + 0.1*((P3^2+P4)*P4.^0.5)/(4*P3^2+P4));
        P2= 0.1*P1;    
        P0= [P1,P2,P3,P4];    %parameters for Lorentzian fit
        BOUNDS= [0,0,0,0;Inf,Inf,Inf,Inf];
        [SED_fit(index(1,i):index(2,i),i), params(:,i), resnormal(i), residual(index(1,i):index(2,i),i)] = SEDfit_func(freqs(index(1,i):index(2,i)),log_SED(index(1,i):index(2,i),i),P0,BOUNDS,'logfull');
...
gamma= c_light*(abs(params(4,:)).^0.5); % gamma is value of P3, unit converted
...
filename=strcat('/home/gkumar1/Phonon_Lifetimes/PhononSED/',material,'_N',num2str(Nunitcells),'_k',num2str(Nk),'/',material,'_',type,'_','fraction',fraction_str,'_','N',num2str(Nunitcells),'_','k',k_no);
...
x= w_mid_rad_total';
y= Tau_total';
% filename=strcat('/home/gkumar1/Phonon_Lifetimes/PhononSED/',material,'_N',num2str(Nunitcells),'_k',num2str(Nk),'/',material,'_',type,'_','ntest','_','fraction',fraction_str,'_','N',num2str(Nunitcells),'_','k',num2str(Nk));
